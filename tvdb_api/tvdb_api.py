...
class Tvdb:
    ...
    def _loadUrl(self, url, data=None, recache=False, language=None):
        ...
        if error:
            if error == "Resource not found":
                ...
                raise tvdb_shownotfound(f"{error}")
            else:
                raise tvdb_error(f"{error}")
    ...
    def search(self, series):
        ...
        """
        series = quote(series.encode("utf-8"))
    ...
    def _getShowData(self, sid, language):
        ...
        for cur_ep in eps_resp:
            ...
            if elem_seasnum is None or elem_epno is None:
                LOG.warning(
                    "An episode has incomplete season/episode number (season: %r, episode: %r)", elem_seasnum, elem_epno
                )
