...
- 1.2.22 (2026.03.28)
    - evvaW
        - 태그 검색 추가
        - 로고 이미지 포함
        - 빈 출연진 정보가 입력되는 문제 수정
        - 기본 장르 및 국가 정보 추가
        - 파일명에 contentid 포함 설정 추가
        - 크리덴셜 기타 설정 추가
        - API 변경 대응
    - TMDB
        - 이미지 사이즈 설정 추가
...
<br><br>
- 1.2.19 (2025.12.17)
    - Wavve: 에피소드 번호가 없고 방영일이 동일한 VOD 파일명 예외 처리
    - Wavve: 재로그인 조건 수정
    - Wavve: 구버전 채널 조회 API 제외
<br><br>
- 1.2.18 (2025.12.13)
    - Wavve: 복수의 계정 정보 설정
    - Wavve: `탐색 일 수` 계산 오류 수정
    - Wavve: 오타 수정
    - Daum: 프로그램 제목에 복수의 프로그램 존재 시 마지막 선택
<br><br>
- 1.2.17 (2025.12.05)
    - Daum: 시리즈 전체 목록이 표시되는 페이지를 파싱
    - Daum: 제목에 시즌 정보가 있으면 추출
    - Wavve: 로그인 API 변경
    - Wavve: 최근방송용 최신 VOD API 변경
    - Wavve: 기타 구버전 API를 호출하지 않도록 수정
<br><br>
- 1.2.16 (2025.11.24)
